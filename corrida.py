...
To play yourself (it's rather fast for humans), type:python carracing.py
...
class CarRacing(gym.Env, EzPickle):
    ...
    def render_indicators(self, W, H):
        ...
        vl.draw(gl.GL_QUADS)
        vl.delete()
        self.score_label.text = "%04i" % self.reward+ " - %i " % self.frame
...
if __name__ == "__main__":
    ...
    # GRAVACAO DOS VIDEOS
    # gravacao -> resolvi separar, esta no arquivo grava_imagens.py
    # para gravar = True senão False
    record_video = False 

    if record_video:
        from gym.wrappers.monitoring import video_recorder
        vid = video_recorder.VideoRecorder(env, path="gravacoes/video.mp4")
